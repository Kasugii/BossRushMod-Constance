// After a PuppetHand dies in boss rush, two things can happen
...
namespace BossRushMod
{
    ...
    {
        /// <summary>True during a boss rush fight against a PuppetHand.
        /// Armed in BossRushArenaForcer.Start(), turned off after use.</summary>
    ...
    // PATCH 2 — Blocks OnHandDeath() → TeleportToShrine (CASE A)
    // OnHandDeath() launches a navigator that moves the player to the VOID exit,
    // then InitTransition to the next VOID. We replace all of that with TeleportToShrine().
    ...
    public static class PuppetHandDeathRedirectPatch
    {
        ...
        {
            if (!PuppetHandRushState.Active) return true;
    ...
    // Prevents ArenaSequence from launching an InitTransition to the next VOID
    // even if OnHandDeath was already blocked (safety CASE A).
    ...
    // When FindPuppetHandDeathTeleport returns empty, ArenaSequence calls
    // StartArenaAndWaitToFinish(player, puppetMaster) which emits
    // ConEntitySignals.ArenaStarted on PuppetMaster, starting its state machine via StartImpl().
    // We intercept StartImpl() and return WaitForArena (inert state) to avoid crashes —
    // the scene will change anyway.
    [HarmonyPatch(
    ...
    public static class PuppetMasterStartImplBlockPatch
    {
            ...
            ref IConStateUpdate __result)
        {
            ...
            // StartImpl() normally hides parkour + laser + hitbox before the fight.
            // Since we block it entirely, we do it manually to avoid
            // the final boss scenery remaining displayed statically.
            try
            {
