...
namespace BossRushMod
{
    ...
    public class BossRushLeaderboardUI : MonoBehaviour
    {
        ...
        public void ShowResults(float time, int count, int listNum)
        {
            ...
            _runCount = count;
            _runListNum = listNum;
            _runCategory = (count == 16) ? 16 : (count == 4) ? 4 : count * 10 + listNum;
            _showResults = true;
            _showLB = false;
            _submitted = false;
            _submitting = false;
        ...
        private void DrawResults(int id)
        {
            ...
            bool isLB = (_runCategory == 16 || _runCount == 4 || _runCount == 5 || _runCount == 10);
        ...
        private void DrawLeaderboard(int id)
        {
            ...
            string lbTitle = (_lbCount == 4)
                ? "LEADERBOARD  —  DESTINATION FINALE"
                : "LEADERBOARD  —  " + _lbCount + " BOSSES  •  " + listName.ToUpper();
            GUI.color = C_GOLD;
            GUILayout.Label(lbTitle, _sTitle);
            GUI.color = p;
            GUILayout.Space(6);

            // ── Onglets : 5 / 10 / 17 ─────────────────────────────────
            GUILayout.BeginHorizontal();
            foreach (int cnt in new[] { 4, 5, 10, 16 })
            {
                bool sel = _lbCount == cnt;
                GUI.color = sel ? C_GOLD : C_GREY;
                string tabLabel = cnt == 4 ? "Dest. Finale" : cnt + " boss";
                float tabW = cnt == 4 ? 90f : cnt == 16 ? 70f : 55f;
                if (GUILayout.Button(tabLabel, sel ? _sTabSel : _sTab,
                        GUILayout.Height(26), GUILayout.Width(tabW)))
                {
                    _lbCount = cnt;
                    _lbListNum = (cnt == 16 || cnt == 4) ? 0 : 1;
                    _lbCategory = (cnt == 16) ? 16 : (cnt == 4) ? 4 : cnt * 10 + _lbListNum;
                    LoadEntries(_lbCategory);
                }
            }
