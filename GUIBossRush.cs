        ...
        public static void TeleportToShrine()
        {
            try
            {
                ...
                var cmd = new ConTransitionCommand_Default(
                    ...
                    true, false, ConRespawnOrigin.Cocoon, null, default(ConCheckPointId), null);
    ...
    public class BossRushUI : BaseUnityPlugin
    {
        ...
        void DrawRushModeSection()
        {
            ...
            if (_rushSubPage == 0)
            {
                ...
                // Destination Finale — fixed order : Corrupted Mind → Forsaken Will → Wounded Vessel → Final Boss
                DrawModeButton("DESTINATION FINALE — 4 last bosses, fixed order", new Color(0.55f, 0f, 0.2f), () =>
                {
                    BossRushModeManager.StartRun(4, 1);
                    CloseGui();
                    ShowStatus("Destination Finale!");
                });
