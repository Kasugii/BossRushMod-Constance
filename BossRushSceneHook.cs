...
namespace BossRushMod
{
    public class BossRushSceneHook : MonoBehaviour
    {
        ...
        {
            // Only act on actual level scenes (Prod_XXX).
            // Ignore ProdMainScene, ProdStartMenu, Boot, etc.
            if (!scene.name.StartsWith("Prod_")) return;
    ...
    public static class FlashbackPatch
    {
        ...
        public static bool PrefixTransitionInit(ref IConTransitionCommand transitionCommand, float delay)
        {
            ...
            try
            {
                // Only intercept transitions that originate from an actual level scene.
                // In ProdMainScene (main scene hub) all transitions are vanilla — new game,
                // loading a save, going to intro — we must never block them.
                string currentScene = UnityEngine.SceneManagement.SceneManager.GetActiveScene().name;
                if (!currentScene.StartsWith("Prod_") || currentScene == "ProdMainScene")
                    return true;

                // The game intro uses CConFlashbackIntroCutsceneController (not a real flashback).
                // If it's present in the scene, let the transition pass through untouched
                // so new save files can be created normally.
                if (UnityEngine.Object.FindObjectOfType<CConFlashbackIntroCutsceneController>() != null)
                    return true;
    ...
    // Patches ConState_Player_GameOver.Enter BEFORE the game freezes and the panel opens.
    // In rush mode, we skip GameOver entirely and teleport to shrine immediately.
    // This must happen here — once AddPauseState is called the transition can no longer fire.
    [HarmonyPatch(typeof(ConState_Player_GameOver), "Enter")]
    public static class PlayerGameOverEnterPatch
    {
        [HarmonyPrefix]
        public static bool Prefix(ref IConStateUpdate __result)
        {
            if (!BossRushModeManager.IsActive) return true; // vanilla

            Debug.Log("[PlayerGameOverPatch] Rush mode → skip GameOver, TeleportToShrine.");
            BossRushModeManager.OnPlayerDied();
            BossRushUtils.TeleportToShrine();
            __result = null;
            return false; // blocks Enter entirely — no pause, no panel
        }
    }
