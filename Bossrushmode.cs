...
namespace BossRushMod
{
    ...
    public static class BossLists
    {
        ...
        // Destination Finale — fixed order, no shuffle
        public static readonly string[] DestinationFinale =
        {
        ...
        {
            if (count == 4) return "Destination Finale";
        ...
        {
            if (count == 4) return DestinationFinale;
    ...
    public static class BossRushModeManager
    {
        ...
        public static bool IsDeathTeleport = false;
        ...
        public static int LeaderboardKey =>
            (TargetCount == 16) ? 16 : (TargetCount == 4) ? 4 : TargetCount * 10 + CurrentListNum;
        public static bool IsLeaderboardRun =>
            (TargetCount == 4 || TargetCount == 5 || TargetCount == 10 || TargetCount == 16);

        public static readonly int[] LeaderboardCategories = { 4, 5, 10, 16 };
        ...
        private static List<string> BuildSequence(int count, int listNum)
        {
            ...
            if (preset != null)
            {
                ...
                // Destination Finale must always run in its fixed order
                if (count != 4) Shuffle(list);
                return list;
            }
        ...
        private void CheckGameOverUI()
        {
            try
            {
                ...
                if (isActive && !_gameOverWasActive)
                {
                    ...
                    Debug.Log("[RushMode] CConUiPanel_GameOver fallback → run annulé.");
